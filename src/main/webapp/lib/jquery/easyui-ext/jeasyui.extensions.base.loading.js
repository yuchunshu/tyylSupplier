/**
 * jQuery EasyUI 1.4.3 Copyright (c) 2009-2015 www.jeasyui.com. All rights reserved.
 * 
 * Licensed under the GPL license: http://www.gnu.org/licenses/gpl.txt To use it on other terms please contact us at
 * info@jeasyui.com http://www.jeasyui.com/license_commercial.php
 * 
 * jQuery EasyUI 通用插件基础库 扩展 jeasyui.extensions.base.loading.js 开发 流云 由 落阳 整理 最近更新：2015-10-25
 * 
 * 依赖项： 1、jquery.jdirk.js 2、jeasyui.extensions.base.css
 * 
...
(function($) {
    ...
    $.easyui.loading = function(options) {
        var opts = $.extend({}, $.easyui.loading.defaults, options || {}), jq = opts.topMost ? $.util.$ : $, locale = jq(opts.locale), zindex = opts.zindex;
        ...
        var container = jq("<div class=\"loading-container\"></div>").css("z-index", ++zindex).appendTo(locale), mask = jq(
                "<div class=\"loading-mask datagrid-mask\"></div>").css("z-index", ++zindex).appendTo(container), msg = jq(
                "<div class=\"loading-message datagrid-mask-msg\"></div>").css("z-index", ++zindex).html(opts.msg)
                .appendTo(container);
    ...
    $.easyui.loaded = function(locale, topMost) {
        ...
        if (arguments.length == 1) {
            var val = arguments[0], type = typeof (val);
            if (type == "boolean") {
                $.extend(opts, {
                    topMost : val
                });
    ...
    $.extend($.fn.panel.defaults, {
        loadingMessage : $.easyui.loading.defaults.msg
    });
