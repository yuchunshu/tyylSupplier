/**
 * jQuery JavaScript Library v1.9.x && v2.x http://jquery.com/
 * 
 * Copyright 2005, 2015 jQuery Foundation, Inc. and other contributors Released under the MIT license
 * http://jquery.org/license
 * 
 * jQuery Extensions Basic Library 基础函数工具包 v1.0 beta jquery.jdirk.js 作者 陈建伟 最近更新：2015-07-14
 * 
 * 依赖项：jquery 1.11.x/2.1.x late
 * 
 * Copyright 2013-2015 ChenJianwei personal All rights reserved. http://www.chenjianwei.org
 */
(function(window, $, undefined) {
    ...
    var document = coreUtil.document = window.document, location = coreUtil.location = window.location, docElem = coreUtil.docElem = document.documentElement, history = coreUtil.history = window.history, parent = coreUtil.parent = window.parent, top = coreUtil.top = window.top, $$ = coreJquery.emptyJquery = coreJquery.empty$ = coreJquery.$$ = coreUtil.emptyJquery = coreUtil.empty$ = coreUtil.$$ = $(), core_string = version,
    ...
    core_push = core_array.push, core_slice = core_array.slice, core_splice = core_array.splice,
    ...
    coreUtil.isJqueryObject = function(obj) {
        return obj != null && obj != undefined && ((obj.jquery ? true : false) || obj.constructor === $$.constructor);
    };
    ...
    coreUtil.parseJqueryObject = coreUtil.parseJquery = function(obj) {
    ...
    coreUtil.resolveClientUrl = coreUtil.resolveUrl = function(url) {
        url = String(url);
        if (coreString.isNullOrEmpty(url)) {
            return url;
        }
        if (coreString.isUrl(url)) {
            return url;
        }
    ...
    // undelayJust: 可选。Boolean 类型值，表示要取消的延迟函数是否为即时调用函数（即通过 coreUtil.delay / $.util.delay 设定该函数时未传入 millisec 值或 millisec
    // 值为 0。）；该参数默认为 false。
    ...
    // 格式如 { code: function|string , error: function|string , final: function|string , tryError: boolean , tryFinal:
    // boolean }
    ...
    // function (code, error, final): 该重载将会被自动转换成 function ({ code: code, error: error, final: finall, tryError: false,
    // tryFinal: false }) 方式执行；
    ...
    coreUtil.tryExec = function(code, error, final) {
        ...
        var opts = $.extend(defaults, typeof code == "object" ? code : {
            code : code,
            error : error,
            final : final
        }), ret;
    ...
    coreUtil.filterProperties = function(obj, propertieNames, excluding) {
        ...
        var ret = {};
        for ( var k in obj) {
            if (excluding ? coreArray.contains(propertieNames, k) : !coreArray.contains(propertieNames, k)) {
                ret[k] = obj[k];
            }
        }
        return ret;
    ...
    $.each([
            {
                unit : String.prototype,
                prototype : coreString,
                methods : [ "anchor", "big", "blink", "bold", "charAt", "charCodeAt", "concat", "fixed", "fontcolor",
                        ...
                        "toLocaleLowerCase", "toLocaleString", "toLocaleUpperCase", "toLowerCase", "toUpperCase",
                        "toString", "trim" ]
            },
            {
                unit : Date.prototype,
                ...
                methods : [ "getDate", "getDay", "getFullYear", "getHours", "getMilliseconds", "getMinutes",
                        ...
                        "getUTCSeconds", "getVarDate", "getYear", "setDate", "setFullYear", "setHours",
            ...
            {
                unit : Number.prototype,
                prototype : coreNumber,
                methods : [ "toExponential", "toFixed", "toLocaleString", "toPrecision", "toString" ]
            },
            {
                unit : Array.prototype,
                prototype : coreArray,
                methods : [ "concat", "every", "filter", "forEach", "indexOf", "join", "lastIndexOf", "map", "pop",
                        "push", "reduce", "reduceRight", "reverse", "shift", "slice", "some", "sort", "splice",
                        "toLocaleString", "toString", "unshift" ]
            }, {
                unit : Boolean.prototype,
                prototype : coreBoolean,
                methods : [ "toString" ]
            } ], function(i, n) {
    ...
    coreString.prototype.isNullOrEmpty = function() {
        return coreString.isNullOrEmpty(this);
    };
    ...
    coreString.prototype.isNullOrWhiteSpace = function() {
        return coreString.isNullOrWhiteSpace(this);
    };
    ...
    coreString.prototype.isHtmlText = function() {
        return coreString.isHtmlText(this);
    };
    ...
    coreString.prototype.replaceAll = function(substr, replacement) {
        return coreString.replaceAll(this, substr, replacement);
    };
    ...
    coreString.format = function(str, arg1, arg2, arg3, argn) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        var isArray = coreUtil.likeArrayNotString(arg1), data = (isArray && !coreUtil.isString(arg1))
                || coreUtil.isObject(arg1) ? arg1 : coreArray.range(arguments, 1);
        if (isArray) {
    ...
    coreString.getByteLen = function(str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        var bytelen = 0, i = 0, length = str.length, charset = (document.charset || "utf-8").toLowerCase(), s = charset == "iso-8859-1" ? 5
                : 2;
        for (; i < length; i++) {
    ...
    coreString.prototype.getByteLen = function() {
        return coreString.getByteLen(this);
    };
    ...
    coreString.substrByte = function(str, index, byteLength) {
        ...
        var bytelen = 0, i = index, length = str.length, charset = (document.charset || "utf-8").toLowerCase(), s = charset == "iso-8859-1" ? 5
                : 2;
        for (; i < length; i++) {
            bytelen += str.charCodeAt(i) > 255 ? s : 1;
    ...
    coreString.prototype.substrByte = function(index, byteLength) {
        return coreString.substrByte(this, index, byteLength);
    };
    ...
    coreString.prototype.contains = function(val) {
        return coreString.contains(this, val);
    };
    ...
    coreString.prototype.reverse = function() {
        return coreString.reverse(this);
    };
    ...
    coreString.prototype.ltrim = function() {
        return coreString.ltrim(this);
    };
    ...
    coreString.prototype.rtrim = function() {
        return coreString.rtrim(this);
    };

    // 去除字符串左右两边的空格；该方法将返回源字符串处理后的一个副本，而不会改变源字符串的值。
    coreString.trim = coreString.trim ? coreString.trim : coreUtil.trim;
    coreString.prototype.trim = function() {
        return coreString.trim(this);
    };
    ...
    coreString.prototype.padLeft = function(len, paddingChar) {
        return coreString.padLeft(this, len, paddingChar);
    };
    ...
    coreString.prototype.padRight = function(len, paddingChar) {
        return coreString.padRight(this, len, paddingChar);
    };
    ...
    coreString.prototype.mid = function(start, len) {
        return coreString.mid(this, start, len);
    };
    ...
    coreString.prototype.lengthOfPrint = function() {
        return coreString.lengthOfPrint(this);
    };
    ...
    coreString.prototype.startsWith = function(val) {
        return coreString.startsWith(this, val);
    };
    ...
    coreString.prototype.endsWith = function(val) {
        return coreString.endsWith(this, val);
    };
    ...
    coreString.prototype.left = function(len) {
        return coreString.left(this, len);
    };
    ...
    coreString.prototype.right = function(len) {
        return coreString.right(this, len);
    };
    ...
    coreString.prototype.leftBytes = function(len) {
        return coreString.leftBytes(this, len);
    };
    ...
    coreString.prototype.rightBytes = function(len) {
        return coreString.rightBytes(this, len);
    };
    ...
    coreString.prototype.isLongDate = function() {
        return coreString.isLongDate(this);
    };
    ...
    coreString.prototype.isShortDate = function() {
        return coreString.isShortDate(this);
    };
    ...
    coreString.prototype.isDate = function() {
        return coreString.isDate(this);
    };
    ...
    coreString.prototype.isTel = function() {
        return coreString.isTel(this);
    };
    ...
    coreString.prototype.isMobile = function() {
        return coreString.isMobile(this);
    };
    ...
    coreString.prototype.isTelOrMobile = function() {
        return coreString.isTelOrMobile(this);
    };
    ...
    coreString.prototype.isFax = function() {
        return coreString.isFax(this);
    };
    ...
    coreString.prototype.isEmail = function() {
        return coreString.isEmail(this);
    };
    ...
    coreString.prototype.isZipCode = function() {
        return coreString.isZipCode(this);
    };
    ...
    coreString.prototype.existChinese = function() {
        return coreString.existChinese(this);
    };
    ...
    coreString.prototype.isChinese = function() {
        return coreString.isChinese(this);
    };
    ...
    coreString.prototype.isEnglish = function() {
        return coreString.isEnglish(this);
    };
    ...
    coreString.prototype.isFileName = function() {
        return coreString.isFileName(this);
    };
    ...
    coreString.isIPv4 = function(str) {
        ...
        if (reSpaceCheck.test(str)) {
            ...
            if (RegExp.$1 <= 255 && RegExp.$1 >= 0 && RegExp.$2 <= 255 && RegExp.$2 >= 0 && RegExp.$3 <= 255
    ...
    coreString.prototype.isIPv4 = function() {
        return coreString.isIPv4(this);
    };
    ...
    coreString.prototype.isUrl = function() {
        return coreString.isUrl(this);
    };
    ...
    coreString.prototype.isUrlOrIPv4 = function() {
        return coreString.isUrlOrIPv4(this);
    };
    ...
    coreString.prototype.isCurrency = function() {
        return coreString.isCurrency(this);
    };
    ...
    coreString.prototype.isQQ = function() {
        return coreString.isQQ(this);
    };
    ...
    coreString.prototype.isMSN = function() {
        return coreString.isMSN(this);
    };
    ...
    coreString.prototype.isUnNormal = function() {
        return coreString.isUnNormal(this);
    };
    ...
    coreString.prototype.isCarNo = function() {
        return coreString.isCarNo(this);
    };
    ...
    coreString.prototype.isCarEngineNo = function() {
        return coreString.isCarEngineNo(this);
    };
    ...
    coreString.prototype.isUserName = function() {
        return coreString.isUserName(this);
    };

    // 判断当前 String 对象是否是正确的 身份证号码(中国) 格式。
    coreString.isIDCard = function(str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        var iSum = 0, sId = str, aCity = {
            11 : "北京",
            12 : "天津",
            13 : "河北",
            14 : "山西",
        ...
        var sBirthday = sId.substr(6, 4) + "-" + Number(sId.substr(10, 2)) + "-" + Number(sId.substr(12, 2)), d = new Date(
                sBirthday.replace(/-/g, "/"));
        // 非法生日
    ...
    coreString.prototype.isIDCard = function() {
        return coreString.isIDCard(this);
    };
    ...
    coreString.isIDCard15 = function(str) {
        ...
        var sYear = "19", sBirthday = sYear + sId.substr(6, 2) + "-" + Number(sId.substr(8, 2)) + "-"
                + Number(sId.substr(10, 2)), d = new Date(sBirthday.replace(/-/g, "/"));
        // 非法生日
        if (sBirthday != (d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate())) {
            return false;
        }
    ...
    coreString.prototype.isIDCard15 = function() {
        return coreString.isIDCard15(this);
    };
    ...
    coreString.prototype.isInteger = function() {
        return coreString.isInteger(this);
    };
    ...
    coreString.prototype.isNumeric = function(flag) {
        return coreString.isNumeric(this, flag);
    };
    ...
    coreString.prototype.isColor = function() {
        return coreString.isColor(this);
    };
    ...
    coreString.prototype.isSafePassword = function() {
        return coreString.isSafePassword(this);
    };
    ...
    coreString.prototype.toCase = function() {
        return coreString.toCase(this);
    };
    ...
    coreString.prototype.toHtmlEncode = function() {
        return coreString.toHtmlEncode(this);
    };
    ...
    coreString.prototype.toDate = function() {
        return coreString.toDate(this);
    };
    ...
    coreString.prototype.toBoolean = function() {
        return coreString.toBoolean(this);
    };
    ...
    coreString.prototype.toInteger = function() {
        return coreString.toInteger(this);
    };
    ...
    coreString.prototype.toNumber = function() {
        return coreString.toNumber(this);
    };
    ...
    coreString.prototype.toFloat = function() {
        return coreString.toFloat(this);
    };
    ...
    coreString.prototype.toNumeric = function() {
        return coreString.toNumeric(this);
    };
    ...
    coreString.prototype.toObject = function() {
        return coreString.toObject(this);
    };
    ...
    coreString.prototype.toJSONString = function() {
        return coreString.toJSONString(this);
    };
    ...
    coreString.prototype.toFunction = function() {
        return coreString.toFunction(this);
    };
    ...
    coreNumber.prototype.round = function(precision) {
        return coreNumber.round(this, precision);
    };
    ...
    coreNumber.prototype.isOdd = function() {
        return coreNumber.isOdd(this);
    };
    ...
    coreNumber.prototype.isEven = function() {
        return coreNumber.isEven(this);
    };
    ...
    coreArray.prototype.isNullOrEmpty = function() {
        return coreArray.isNullOrEmpty(this);
    };
    ...
    coreArray.prototype.copy = coreArray.prototype.copyFrom = function(source) {
    ...
    coreArray.prototype.copyTo = function(target) {
        return coreArray.copy(target, this);
    };
    ...
    coreArray.prototype.clone = function() {
        return coreArray.clone(this);
    };
    ...
    coreArray.prototype.contains = function(item, compare) {
        return coreArray.contains(this, item, compare);
    };
    ...
    coreArray.prototype.indexOf = function(item, startIndex, count, compare) {
        return coreArray.indexOf(this, item, startIndex, count, compare);
    };
    ...
    function(array, item, startIndex, count, compare) {
        ...
        var begin = array.length - startIndex - 1, end = begin - count;
    ...
    coreArray.prototype.lastIndexOf = function(item, startIndex, count, compare) {
        return coreArray.lastIndexOf(this, item, startIndex, count, compare);
    };
    ...
    coreArray.prototype.range = function(startIndex, stopIndex) {
        return coreArray.range(this, startIndex, stopIndex);
    };
    ...
    coreArray.prototype.rangeLen = function(startIndex, length) {
        return coreArray.rangeLen(this, startIndex, length);
    };
    ...
    coreArray.prototype.splitPage = function(pageIndex, pageSize, sortby) {
        return coreArray.splitPage(this, pageIndex, pageSize, sortby);
    };
    ...
    coreArray.prototype.removeRange = function(index, count) {
        return coreArray.removeRange(this, index, count);
    };
    ...
    coreArray.prototype.clear = function() {
        return coreArray.clear(this);
    };
    ...
    coreArray.prototype.removeAt = function(index) {
        return coreArray.removeAt(this, index);
    };
    ...
    coreArray.prototype.remove = function(item, compare) {
        return coreArray.remove(this, item, compare);
    };
    ...
    coreArray.prototype.insertRange = function(index, collect) {
        return coreArray.insertRange(this, index, collect);
    };
    ...
    coreArray.prototype.insert = function(index, item) {
        return coreArray.insert(this, index, item);
    };
    ...
    coreArray.prototype.setRange = function(index, collect) {
        return coreArray.setRange(this, index, collect);
    };
    ...
    coreArray.prototype.attach = function(item, compare) {
        return coreArray.attach(this, item, compare);
    };
    ...
    coreArray.prototype.distinct = function(compare) {
        return coreArray.distinct(this, compare);
    };
    ...
    coreArray.prototype.filter = function(compare) {
        return coreArray.filter(this, compare);
    };
    ...
    coreArray.prototype.map = function(callback, thisArg) {
        return coreArray.map(this, callback, thisArg);
    };
    ...
    coreArray.prototype.max = function(compare) {
        return coreArray.max(this, compare);
    };
    ...
    coreArray.prototype.maxs = function(compare) {
        return coreArray.maxs(this, compare);
    };
    ...
    coreArray.prototype.min = function(compare) {
        return coreArray.min(this, compare);
    };
    ...
    coreArray.prototype.mins = function(compare) {
        return coreArray.mins(this, compare);
    };
    ...
    coreArray.sum = function(array, callback, thisArg) {
        var isFunc = coreUtil.isFunction(callback), fn = function(previous, current) {
            return previous + (isFunc ? callback.call(thisArg, current) : current);
        };
        return coreArray.reduce(array, fn, 0);
    };
    coreArray.prototype.sum = function(convert) {
        return coreArray.sum(this, convert);
    };

    // 计算数组中各项累积后的平均值；该函数参数的定义和 coreArray.sum 一样；
    coreArray.avg = function(array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        var sum = coreArray.sum(array, callback, thisArg), avg = parseFloat(sum) / array.length;
        return avg;
    };
    coreArray.prototype.avg = function(convert) {
        return coreArray.avg(this, convert);
    };
    ...
    coreArray.prototype.take = function(count) {
        return coreArray.take(this, count);
    };
    ...
    coreArray.prototype.skip = function(count) {
        return coreArray.skip(this, count);
    };
    ...
    coreArray.prototype.forEach = function(callback, thisArg) {
        return coreArray.forEach(this, callback, thisArg);
    };
    ...
    coreArray.prototype.reduce = function(callback, initialValue) {
        return coreArray.reduce(this, callback, initialValue);
    };
    ...
    coreArray.prototype.reduceRight = function(callback, initialValue) {
        return coreArray.reduceRight(this, callback, initialValue);
    };
    ...
    coreArray.prototype.every = function(callback, thisArg) {
        return coreArray.every(this, callback, thisArg);
    };
    ...
    // 备注：some 方法会按升序索引顺序对每个数组元素调用 callbackfn 函数，直到 callbackfn 函数返回 true。 如果找到导致 callbackfn 返回 true 的元素，则 some 方法会立即返回
    // true。 如果回调不对任何元素返回 true，则 some 方法会返回 false。
    ...
    coreArray.prototype.some = function(callback, thisArg) {
        return coreArray.some(this, callback, thisArg);
    };
    ...
    // 备注：first 方法会按升序索引顺序对每个数组元素调用 callbackfn 函数，直到 callbackfn 函数返回 true。 如果找到导致 callbackfn 返回 true 的元素，则 first
    // 方法会立即返回该元素。 如果回调不对任何元素返回 true，则 first 方法会返回 undefined。
    ...
    coreArray.prototype.first = function(callback, thisArg) {
        return coreArray.first(this, callback, thisArg);
    };
    ...
    // 备注：last 方法会按降序索引顺序对每个数组元素调用 callbackfn 函数，直到 callbackfn 函数返回 true。 如果找到导致 callbackfn 返回 true 的元素，则 last
    // 方法会立即返回该元素。 如果回调不对任何元素返回 true，则 last 方法会返回 undefined。
    ...
    coreArray.prototype.last = function(callback, thisArg) {
        return coreArray.last(this, callback, thisArg);
    };
    ...
    coreArray.prototype.left = function(length) {
        return coreArray.left(this, length);
    };
    ...
    coreArray.prototype.right = function(length) {
        return coreArray.right(this, length);
    };
    ...
    coreDate.prototype.isLeapYear = function() {
        return coreDate.isLeapYear(this);
    };
    ...
    coreDate.prototype.compareTo = function(date) {
        return coreDate.compare(this, date);
    };
    coreDate.prototype.equals = function(date) {
        return coreDate.compare(this, date) == 0;
    };
    ...
    coreDate.prototype.getQuarter = function() {
        return coreDate.getQuarter(this);
    };
    ...
    coreDate.prototype.getDayOfWeek = function() {
        return coreDate.getDayOfWeek(this);
    };
    ...
    coreDate.getWeek = function(date, weekStart) {
        ...
        var year = date.getFullYear(), firstDay = new Date(year, 0, 1), firstWeekDays = 7 - firstDay.getDay()
                + weekStart, dayOfYear = (((new Date(year, date.getMonth(), date.getDate())) - firstDay) / (24 * 3600 * 1000)) + 1;
        return Math.ceil((dayOfYear - firstWeekDays) / 7) + 1;
    };
    coreDate.prototype.getWeek = function(weekStart) {
        return coreDate.getWeek(this, weekStart);
    };
    ...
    coreDate.getWeekOfMonth = function(date, weekStart) {
        ...
        var dayOfWeek = date.getDay(), day = date.getDate();
        return Math.ceil((day - dayOfWeek - 1) / 7) + ((dayOfWeek >= weekStart) ? 1 : 0);
    };
    coreDate.prototype.getWeekOfMonth = function(weekStart) {
        return coreDate.getWeekOfMonth(this, weekStart);
    };
    ...
    coreDate.prototype.addTime = function(millisec) {
        return coreDate.addTime(this, millisec);
    };
    ...
    coreDate.prototype.addDays = function(days) {
        return coreDate.addDays(this, days);
    };
    ...
    coreDate.prototype.addHours = function(hours) {
        return coreDate.addHours(this, hours);
    };
    ...
    coreDate.prototype.addMilliseconds = function(millisec) {
        return coreDate.addMilliseconds(this, millisec);
    };
    ...
    coreDate.prototype.addMinutes = function(minutes) {
        return coreDate.addMinutes(this, minutes);
    };
    ...
    coreDate.prototype.addWeeks = function(weeks) {
        return coreDate.addWeeks(this, weeks);
    };
    ...
    coreDate.addMonths = function(date, months) {
        ...
        return new Date(date.getFullYear(), date.getMonth() + months, date.getDate(), date.getHours(), date
                .getMinutes(), date.getSeconds(), date.getMilliseconds());
    };
    coreDate.prototype.addMonths = function(months) {
        return coreDate.addMonths(this, months);
    };
    ...
    coreDate.prototype.addSeconds = function(seconds) {
        return coreDate.addSeconds(this, seconds);
    };
    ...
    coreDate.prototype.addTicks = function(ticks) {
        return coreDate.addTicks(this, ticks);
    };
    ...
    coreDate.addYears = function(date, years) {
        ...
        return new Date(date.getFullYear() + years, date.getMonth(), date.getDate(), date.getHours(),
                date.getMinutes(), date.getSeconds(), date.getMilliseconds());
    };
    coreDate.prototype.addYears = function(years) {
        return coreDate.addYears(this, years);
    };
    ...
    coreDate.addQuarters = function(date, quarters) {
        ...
        return new Date(date.getFullYear(), date.getMonth() + quarters * 3, date.getDate(), date.getHours(), date
                .getMinutes(), date.getSeconds(), date.getMilliseconds());
    };
    coreDate.prototype.addQuarters = function(quarters) {
        return coreDate.addQuarters(this, quarters);
    };
    ...
    coreDate.prototype.add = function(datepart, number) {
        return coreDate.add(this, datepart, number);
    };
    ...
    coreDate.prototype.diff = function(datepart, value) {
        return coreDate.diff(this, datepart, value);
    };
    ...
    coreDate.prototype.part = function(datepart) {
        return coreDate.part(this, datepart);
    };
    ...
    coreDate.prototype.format = function(format) {
        return coreDate.format(this, format);
    };

    // 获取当前日期时间的长字符串格式，返回的日期时间字符串格式如 “2013年05月16日 星期四 夏季, 下午 15:38:11”
    coreDate.toLongDateTimeString = function(date) {
        date = coreDate.toDate(date);
        var year = date.getFullYear(), month = date.getMonth(), day = date.getDate(), hours = date.getHours(), minutes = date
                .getMinutes(), seconds = date.getSeconds(), week = date.getDay(), quarter = coreDate.getQuarter(date), hoursArray = [
                "午夜", "凌晨", "早上", "上午", "中午", "下午", "傍晚", "晚上" ], weekArray = [ "星期日", "星期一", "星期二", "星期三", "星期四",
                "星期五", "星期六" ],
        // monthArray = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"],
        quarterArray = [ "春", "夏", "秋", "冬" ], hourSpan = hoursArray[Math.floor(parseFloat(hours) / 3)], weekSpan = weekArray[week],
        ...
        return coreString.format("{0}年{1}月{2}日 {3} {4}季, {5} {6}:{7}:{8}", year, ("" + (month + 101)).substr(1),
                ("" + (day + 100)).substr(1), weekSpan, quarterSpan, hourSpan, ("" + (hours + 100)).substr(1),
                ("" + (minutes + 100)).substr(1), ("" + (seconds + 100)).substr(1));
    };
    coreDate.prototype.toLongDateTimeString = function() {
        return coreDate.toLongDateTimeString(this);
    };
    ...
    coreUtil.getElementUniqueID = function(element) {
        return element != undefined && element != null && element.getAttribute ? element
                .getAttribute(coreUtil.uniqueIdName) : null;
    };
    ...
    coreUtil.hasParentJquery = function(win) {
        ...
        try {
            var p = win.parent;
            ret = p && coreUtil.isWindow(p) && coreUtil.isObject(p.document) && coreUtil.isFunction(p.jQuery) ? true
                    : false;
        } catch (ex) {
        }
        return ret;
    ...
    coreUtil.getCurrentFrame = function() {
        ...
        var result = null;
        var frames = coreArray.merge([], top.document.getElementsByTagName("iframe"), top.document
        ...
        var find = function(frame) {
            ...
            try {
                ...
                var fs = coreArray.merge([], win.document.getElementsByTagName("iframe"), win.document
                        .getElementsByTagName("frame"));
                $.each(fs, function() {
                    result = find(this);
                    return result == null;
    ...
    coreUtil.windowSize = function() {
        var length = arguments.length, arg1, arg2, arg1Type, arg2Type, getSize = function() {
            var win = $(window);
            return {
    ...
    coreUtil.windowOffset = function() {
        var length = arguments.length, arg1, arg2, arg1Type, arg2Type, getOffset = function() {
            return {
    ...
    coreUtil.windowPosition = function() {
        var length = arguments.length, arg1, arg2, arg3, arg4, arg1Type, arg2Type, getPosition = function() {
        ...
        if (length == 1) {
            ...
            if (coreUtil.isPlainObject(arg1) || arg1Type == "function") {
                coreUtil.position(arg1.width || position.width, arg1.height || position.height, arg1.left
                        || position.left, arg1.top || position.top);
            }
        }
        if (length == 2) {
            arg2 = arguments[1];
            ...
            if (arg1Type == "string" && arg2Type == "number") {
                ...
                if (position.width != newPosition.width || position.height != newPosition.height
                        || position.left != newPosition.left || position.top != newPosition.top) {
                    window.moveTo(newSize.left, newSize.top);
    ...
    coreUtil.requestAjaxBoolean = function(url, args) {
        ...
        if (type == "object"
                || (type == "string" && data.charAt(0) === "<" && data.charAt(data.length - 1) === ">" && data.length >= 3)) {
            data = $(data).text();
        }
    ...
    coreJquery.prototype.parentSize = function() {
        ...
        var t = this[0], p = (t.tagName == "BODY" ? t : this.parent()[0]);
        return {
            width : $(p).width(),
            height : $(p).height()
        }
    ...
    // function() : 获取当前表达式匹配到的第一个元素的边框值，该函数返回一个格式如 { top: number, left: number, right: number, bottom: number } 的 JSON
    // 对象；
    ...
    coreJquery.prototype.border = function(region, val) {
        ...
        if (arguments.length == 1) {
            if (coreUtil.isNumeric(region) || coreString.endsWith(region, "px")) {
                ...
                return coreArray.contains([ "top", "left", "right", "bottom" ], locale) ? getRegionBorder(locale)
                        : null;
            }
        } else {
            var locale = coreString.toLowerCase(region);
    ...
    coreJquery.prototype.hasUniqueID = function() {
        return this.length ? coreUtil.hasUniqueID(this[0]) : false;
    };

    // 获取当前表达式匹配到的所有元素中的第一个元素的 uniqueID 属性值；
    // 返回值：如果当前表达式没有匹配的元素，则返回 null；否则返回 所有元素中的第一个元素的 uniqueID 属性值；
    coreJquery.prototype.getUniqueID = function() {
        return this.length ? coreUtil.getElementUniqueID(this[0]) : null;
    };
    ...
    coreJquery.prototype.setUniqueID = function(uniqueID) {
    ...
    coreJquery.prototype.initUniqueID = function() {
        return this.each(function() {
            coreUtil.initElementUniqueID(this);
        });
    };
    ...
    coreJquery.prototype.serializeObject = function(options) {
        ...
        rsubmittable_radiocheckbox = /^(?:checkbox|radio)$/i, list, names, ret = {};
        options = options || {};
        ...
        var defaults = {
            ...
            overtype : "append",
            separator : ","
        }, opts = $.extend({}, defaults, (typeof options == "string") ? {
            transcript : options
        } : options || {});
        ...
        list = this
                ...
                .map(
                        function(i, elem) {
                            var name = elem.name, id = elem.id, type = this.type, val = $(this).val(), checked = (this.checked == undefined || this.checked == null) ? null
                                    : this.checked;
                            return {
                                name : name || id,
                                type : type,
                                checked : checked,
                                val : $.isArray(val) ? $.map(val, function(val) {
        ...
        function getElemsVal(elems) {
            var items = coreArray.filter(elems, function(elem) {
                return (rsubmittable_radiocheckbox.test(elem.type) && elem.checked == true)
                        || !rsubmittable_radiocheckbox.test(elem.type);
            });
            var values = coreArray.map(items, function(val) {
                return val.val;
            });
            ...
            case "discard":
                return values[0];
            case "overlay":
                return opts.overtype == "array" ? (values.length > 1 ? values : values[0]) : values
                        .join(opts.separator);
            default:
                return values[0];
            }
    ...
    coreUtil.define = function(namespace, data, createFn) {
        ...
        $.extend(func, {
            extend : $.extend,
            union : coreJquery.union,
            init : constructor,
            inst : createFn
        });
        $.extend(func.defaults, data, {
            extend : $.extend,
            union : coreJquery.union
        });
        if (p && name) {
    ...
    coreJquery.prototype.requestFullScreen = function() {
        return coreJquery.requestFullScreen(this);
    };
    ...
    coreJquery.prototype.cancelFullScreen = function() {
        return coreJquery.cancelFullScreen(this);
    };
    ...
    coreJquery.prototype.toggleFullScreen = function() {
        return coreJquery.toggleFullScreen(this);
    };

    // 获取一个 bool 类型值，该值指示当前浏览器是否支持全屏 API。
    coreUtil.supportsFullScreen = fullScreen.supports;
    // 如果 $.util.supportsFullScreen 值为 true，则该属性获取浏览器的全屏 API 事件名称；如果 $.util.supportsFullScreen 值为 false；则该属性返回
    // null/undefined。
    ...
    coreUtil.shine = coreJquery.shine = function(selector, interval, times) {
        ...
        selector = coreUtil.parseJquery(selector);
        if (!coreUtil.isNumeric(interval) || interval <= 40) {
            interval = coreUtil.shineInterval;
        }
        if (!coreUtil.isNumeric(times) || times < 4) {
            times = coreUtil.shineTimes;
        }
        var a = function() {
        ...
        var run = function() {
            ...
            times--;
            if (times > 0) {
                coreUtil.delay(run, interval);
            }
        };
        coreUtil.delay(run);
        return selector;
    };
    coreJquery.prototype.shine = function(interval, times) {
        return coreJquery.shine(this, interval, times);
    };
    ...
    coreUtil.pageNestingExecute = function(callback) {
        ...
        var windowList = [], decline = function(win) {
            ...
            var jq = win.jQuery;
            if (jq) {
                jq("frame,iframe").each(
                        function() {
                            try {
                                if (this.contentWindow && this.contentWindow.document && this.contentWindow.jQuery
                                        && this.contentWindow.jQuery.fn) {
    ...
    // fn : 一个签名为 function (doc, e) 的函数，其中 win 表示所在 iframe 执行函数传入的 window 对象，e 表示最初触发该循环函数调用的事件对象，该函数内的 this
    // 指向该函数本身被调用时的 window 对象。
    ...
    // jQuery.extend 以及 jQuery.fn.extend：无论 target 对象中是否存在 object1、object2、objectN 等待合并对象中相应的属性，待合并对象中的相应属性都将会合并到 target
    // 中；
