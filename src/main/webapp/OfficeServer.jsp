...
<%@page import="DBstep.IDBManagerBean.*"%>
<%!
  public class iWebOffice {
    ...
    private DBstep.IDBManagerBean BeanObj;
    //==============================↓文档、模板管理代码【开始】↓==============================
...
System.out.println("mTemplate=" + mTemplate);
                if (MsgObj.MsgFileLoad(mTemplate)) {
System.out.println("打开模板成功");
              ...
              mRecordID=MsgObj.GetMsgByName("RECORDID");                            //取得文档编号
              mFileName=MsgObj.GetMsgByName("PDFFILENAME");                            //取得文档名称
System.out.println("PDFFILENAME=" + mFileName);
              MsgObj.MsgTextClear();                                                        //清除文本信息
              //if (MsgObj.MsgFileSave(mFilePath+"\\Document\\"+mRecordID+".pdf")){ //保存文档到文件夹中(存在风险)
              //mFileName为绝对路径文件名
...
System.out.println("PDF 转换失败!");
              ...
              if (mDirectory.trim().equalsIgnoreCase("")) {
                ...
                //mDirectory为相对根目录的路径,不包含文件名
                mFilePath = mFilePath + mDirectory;
              }
              ...
              mRecordID = MsgObj.GetMsgByName("RECORDID");                        //取得文档编号
              mFileName = MsgObj.GetMsgByName("SOURCEFILE");                      //取得文档名称
              mFileType = MsgObj.GetMsgByName("FILETYPE");                        //取得文档类型
              MsgObj.MsgTextClear();
              //if (LoadFile()) {                                                   //调入文档
             if (MsgObj.MsgFileLoad(mFileName)) {
                //MsgObj.MsgFileBody(mFileBody);                                    //将文件信息打包
                MsgObj.MsgFileBody(MsgObj.MsgFileBody());
              ...
              //mImageName为相对根目录的路径，包含文件名
              mFilePath = mFilePath + mImageName;
              ...
              //mRecordID为相对根目录的相对路径，包含文件名
              mFilePath = mFilePath + mRecordID + ".pgf";
