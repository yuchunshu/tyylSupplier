 ...
 * 环节实例
 *
 * @author hzr 2016-11-28
 *
 */
@Entity
@AttributeOverrides({ @AttributeOverride(name = "id", column = @Column(name = "node_inst_id")) })
@Table(name = "wf_node_inst")
public class WfNodeInst extends StringUuidEntity {

    /**  */
    private static final long serialVersionUID = 8764299187506566094L;
    ...
    public boolean isSubInst() {
    	return this.parentNodeInstId != null && this.parentNodeInstId.trim().length() > 0;
    }
	...
	public Long getLeftDays() {
        if (this.dealTime == null && this.flowInst.getExpiryDate() != null) {
            return (this.flowInst.getExpiryDate().getTime() - (new Date()).getTime()) / (1000);
        } else {
            return null;
        }
    }
